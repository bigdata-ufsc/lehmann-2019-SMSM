...
public interface MSMMovePointsTest {
	...
	default IMeasureDistance<SemanticTrajectory> measurer(Problem problem) {
		if(problem instanceof NElementProblem) {
			...
			return new MSMMoveClassifier(new MSMMoveSemanticParameter(((PisaProblem) problem).stopSemantic(), null, PisaDataReader.MOVE_POINTS_SEMANTIC, Thresholds.MOVE_INNER_POINTS_PERC, .5)
					, new MSMSemanticParameter<TPoint, Number>(Semantic.GEOGRAPHIC_LATLON, Thresholds.GEOGRAPHIC_LATLON, 1)
					);
		}
		return null;
