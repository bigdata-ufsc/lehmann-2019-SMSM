...
public class MSMClassifierTest {
	...
	private static NearestNeighbour<SemanticTrajectory, Object> nn;
	...
	public static void setupClass() throws IOException, InterruptedException {
		...
		ArrayList<DataEntry<SemanticTrajectory, Object>> entries = new ArrayList<>();
		for (SemanticTrajectory traj : training) {
			entries.add(new DataEntry<>(traj, traj.getDimensionData(7, 0)));
		}
		nn = new NearestNeighbour<SemanticTrajectory, Object>(entries, Math.min(training.size(), 3), new MSMClassifier(), false);
	}
	...
	public void testName() throws Exception {
		...
		for (KNNTestData<SemanticTrajectory, Object> st : testing) {
			Object[] classified = (Object[]) nn.classify(new DataEntry<>(st.getData(), "descubra"));
