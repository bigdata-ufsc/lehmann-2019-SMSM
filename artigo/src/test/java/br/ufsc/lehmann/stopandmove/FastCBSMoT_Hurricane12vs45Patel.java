...
public class FastCBSMoT_Hurricane12vs45Patel {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
		...
		PreparedStatement update = conn.prepareStatement("update patel.hurricane_12vs45 set semantic_stop_id = ?, semantic_move_id = ? where tid = ? and gid in (SELECT * FROM unnest(?))");
...
//			}
	...
	private static Map<String, Integer> findBestCBSMoT(FastCBSMoT fastCBSMoT, List<SemanticTrajectory> trajs, AtomicInteger sid, AtomicInteger mid) {
		...
		for (double i = 4.0; i <= 5.0; i += 0.2) {//ratio
				...
				for (double k = 2.0; k <= 10.0; k += 0.2) {//maxDist
					...
					IntStream.iterate(20, l -> l + 1).limit(15).parallel().forEach((l) -> {//mergeTolerance
						for (int m = 20; m <= 60; m+=5) {//minTime
							...
							String format = String.format("%.2f-%d-%.2f-%d-%d", finalI, finalJ, finalK, l, m);
							bestCombinations.put(format, stopsCount);
							System.out.println(format + " -> " + stopsCount);
						}
					});
				}
			}
