...
/**
 * Clusters store clustering results. Independent of Cytoscape.
 * 
 * @author djh.shih
 *
 */
public class Clusters {
	 ...
	 * 
	 * @param assignments
	 *            cluster index/labels of each element (cluster labels will be re-assigned)
	 * @param k
	 *            number of clusters
	 */
	 ...
	 * 
	 * @param assignments
	 *            cluster index/labels of each element (clusters must be labeled 0 to k-1)
	 * @param k
	 *            number of clusters
	 * @param cost
	...
	/**
	 * Re-assign clusterIndex s.t. clusters are labeled 0 to k-1. Labels assigned in the order in which the cluster appear in the cluster assignment
	 * TODO Ensure stable label assignment? Sort original labels; calculate intervals between labels; iterate through clusterIndex, subtracting
	 * intervals as appropriate; O(n)
	 * 
	 * @param assignments
	 * @param cost
	 */
	protected void init(int[] assignments, double cost) {
		...
		/*
		 * System.out.println("Cluster init assignments:"); for (int ii = 0; ii < m; ++ii) {
	...
	public void order(int[] clusterOrderIndex) {
		...
		if (k != this.k) {
			throw new IllegalArgumentException(
					"clusterOrderIndex must be a permutation of the cluster index (and its size must be equal to the number of clusters).");
		}
