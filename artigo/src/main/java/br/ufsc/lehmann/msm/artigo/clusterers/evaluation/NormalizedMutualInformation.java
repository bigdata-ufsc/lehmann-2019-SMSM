...
import br.ufsc.lehmann.msm.artigo.clusterers.util.DoubleList;
 ...
 * @author Edward Raff
 */
public class NormalizedMutualInformation<T> implements ClusterEvaluation<T> 
{
	private Semantic<Number, T> semantic;

	public NormalizedMutualInformation(Semantic<Number, T> semantic) {
		this.semantic = semantic;
	}

    @Override
    public double evaluate(int[] designations, Trajectories<Number, T> cds)
    {
        double nmiNumer = 0.0;
        double nmiC = 0.0;
        double nmiK = 0.0;
        ...
        for(int i= 0; i < cds.sampleSize(); i++)
        {
            ...
            while(kPriors.size() <= ki)
                kPriors.add(0.0);
            kPriors.set(ki, kPriors.get(ki)+1);
        }
        ...
        for(int i = 0; i < cds.sampleSize(); i++)
        {
            int ci = semantic.getData(cds.get(i), 0).intValue();
