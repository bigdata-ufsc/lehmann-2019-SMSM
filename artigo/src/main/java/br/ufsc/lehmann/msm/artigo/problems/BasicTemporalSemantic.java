...
public class BasicTemporalSemantic<V> extends TemporalSemantic<V, Number> {
	
	private IDistanceFunction<V> distance = new EqualsDistanceFunction<>();

	public BasicTemporalSemantic(int index) {
		this(index, new EqualsDistanceFunction<>());
	}

	public BasicTemporalSemantic(int index, IDistanceFunction<V> distance) {
		super(index);
