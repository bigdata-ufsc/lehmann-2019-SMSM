...
public abstract class LanceWilliamsDissimilarity extends DistanceMetricDissimilarity implements UpdatableClusterDissimilarity
{
     ...
     */
    public LanceWilliamsDissimilarity(IMeasureDistance<SemanticTrajectory> dm)
    {
        super(dm);
    }
    ...
    public double dissimilarity(List<SemanticTrajectory> a, List<SemanticTrajectory> b)
    {
        if(a.size() == 1 && b.size() == 1)
            return dm.distance(a.get(0), b.get(0));
        
        List<SemanticTrajectory> CI;
        List<SemanticTrajectory> CJ;
        List<SemanticTrajectory> CK;
        if(a.size() > 1)
        {
            CI = a.subList(0, 1);
