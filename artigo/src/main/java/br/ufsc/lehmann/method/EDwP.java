...
public class EDwP extends TrajectorySimilarityCalculator<SemanticTrajectory> implements IMeasureDistance<SemanticTrajectory> {
	...
	public double getDistance(SemanticTrajectory s, SemanticTrajectory t) {
		...
		Matrix matrix = new Matrix(t1.edges.size() + 1, t2.edges.size() + 1);
		...
		for (int i = 1; i < matrix.numRows(); i++) {
			for (int j = 1; j < matrix.numCols(); j++) {
				...
				if ((diagScore <= colSpatialScore) && (diagScore <= rowSpatialScore)) {
					...
					matrix.add(i, j, colSpatialScore, (byte) 2, t2Point[j - 1], t1Insert);
					matrix.delta[i][j] = (colSpatialScore - colDelta);
				} else {
					matrix.add(i, j, rowSpatialScore, (byte) 3, t2Insert, t1Point[i - 1]);
	...
	public void printPath(Matrix matrix) {
		matrix.printPath();
	}
