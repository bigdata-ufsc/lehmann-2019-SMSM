...
import br.ufsc.lehmann.msm.artigo.problems.AnimalPatelDataReader;
import br.ufsc.lehmann.msm.artigo.problems.AnimalPatelProblem;
...
public class FastCBSMoT_AnimalsPatel {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
		...
		AnimalPatelProblem problem = new AnimalPatelProblem();
		...
		int timeTolerance = 30; // time in ms dif do ponto final para o inicial deve ser maior que timeTolerance
		...
		int mergeTolerance = 300;// time in ms

		// Clean - Os stops devem ter pelo menos o minTime
		int minTime = 100; // time in ms
...
//				}
//			}
			List<StopAndMove> findBestCBSMoT = findCBSMoT(fastCBSMoT, new ArrayList<>(trajs), 300, 5, 350, 400, 10, sid);
			for (StopAndMove stopAndMove : findBestCBSMoT) {
				List<Stop> stops = stopAndMove.getStops();
				System.out.println("Traj.: " + AnimalPatelDataReader.TID.getData(stopAndMove.getTrajectory(), 0) + ", stops: " + stops.size());
				for (Stop stop : stops) {
