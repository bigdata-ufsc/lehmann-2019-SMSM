...
public class FastCBSMoT_SanFranciscoCab {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
		...
		// FIND STOPS
		double ratio = 250; // distance in meters to find neighbors
		...
		int mergeTolerance = 30 * 1000;// time in ms
		...
		PreparedStatement update = conn.prepareStatement("update public.taxicab_sanfrancisco_crawdad set semantic_stop_id = ?, semantic_move_id = ? where tid = ? and gid in (SELECT * FROM unnest(?))");
...
//			System.out.println(t.get(t.size() - 1));
	...
	public static Map<String, Integer> findBestCBSMoT(FastCBSMoT fastCBSMoT, List<SemanticTrajectory> trajs, AtomicInteger sid, AtomicInteger mid) {
		...
		for (int i = 150; i <= 450; i+=50) {//ratio
			final int finalI = i;
			IntStream.iterate(30 * 1000, j -> j + 20 * 1000).limit(5).parallel().forEach((j) -> {//timeTolerance
				final int finalJ = j;
				for (int k = 500; k <= 800; k+=50) {//maxDist
					final int finalK = k;
					IntStream.iterate(30*1000, l -> l + 20 * 1000).limit(10).parallel().forEach((l) -> {//mergeTolerance
						for (int m = 60 * 1000; m <= 180 * 1000; m+=20000) {//minTime
