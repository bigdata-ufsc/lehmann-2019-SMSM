...
public class FastCBSMoT_VehiclePatel {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
		...
		PreparedStatement update = conn.prepareStatement("update patel.vehicle set semantic_stop_id = ?, semantic_move_id = ? where gid in (SELECT * FROM unnest(?))");
	...
	private static Map<String, Integer> findBestCBSMoT(FastCBSMoT fastCBSMoT, List<SemanticTrajectory> trajs, MutableInt sid) {
		...
		IntStream.iterate(20, i -> i + 4).limit(5).parallel().forEach((j) -> {//timeTolerance
			for (int i = 100; i <= 300; i+=20) {//ratio
				for (int k = 400; k <= 600; k+=25) {//maxDist
					for (int l = 300; l <= 500; l+=25) {//mergeTolerance
