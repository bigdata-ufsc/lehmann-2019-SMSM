...
public class FastCBSMoT_NewYorkBus {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
		...
		ResultSet executeQuery = conn.createStatement().executeQuery("select max(stop_id), max(move_id) from stops_moves.bus_nyc_20140927");
		executeQuery.next();
		MutableInt sid = new MutableInt(executeQuery.getInt(1));
		MutableInt mid = new MutableInt(executeQuery.getInt(2));
		PreparedStatement update = conn.prepareStatement("update bus.nyc_20140927 set semantic_stop_id = ?, semantic_move_id = ? where gid in (SELECT * FROM unnest(?))");
		PreparedStatement insert = conn.prepareStatement("insert into stops_moves.bus_nyc_20140927(stop_id, start_time, start_lat, start_lon, end_time, end_lat, end_lon, centroid_lat, centroid_lon) values (?,?,?,?,?,?,?,?,?)");
		try {
