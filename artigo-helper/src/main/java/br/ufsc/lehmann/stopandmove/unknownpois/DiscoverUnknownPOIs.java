...
public class DiscoverUnknownPOIs {
	...
	public static void main(String[] args) throws InstantiationException, IllegalAccessException, ClassNotFoundException, SQLException {
		...
		PreparedStatement ps = conn.prepareStatement("update involves.\"stops_FastCBSMoT" + YEAR_MONTH + "_" + ratio + "mts_" + stopMinutes + "_mins" + "\" "
	...
	private static void prepareDatabase(int ratio, int stopMinutes, Connection conn) {
		try {
	...
	private static Map<Stop, Object> extractUnknownPOIs(int ratio, int stopMinutes, Connection conn)
		...
		Collection<Stop> values = stops.values().stream().filter((Stop s) -> ((StopData) s.getRegion()).colab_PDV_distance > ratio && ((StopData) s.getRegion()).pDV_distance > ratio ).collect(Collectors.toList());
		Stop[] array = values.toArray(new Stop[values.size()]);
		...
		int minPts = 1;
		Map<Stop, Object> dbscan = discoverUnknownPOIs.dbscan(array, distance, minPts, ratio);
		dbscan.forEach((key, value) -> ((StopData) key.getRegion()).globalUnknownPOIName = (String) value);
		Map<Integer, List<Stop>> grouped = dbscan.keySet().stream().collect(Collectors.groupingBy(Stop::getUser, Collectors.toList()));
