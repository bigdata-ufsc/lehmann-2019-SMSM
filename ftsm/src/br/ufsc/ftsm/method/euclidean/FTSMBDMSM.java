package br.ufsc.ftsm.method.euclidean;
...
public class FTSMBDMSM extends TrajectorySimilarityCalculator<Trajectory> {
		...
		while (!queue.isEmpty()) {
			...
			if (!node.isLeaf) {
				...
				if (!matchingList.isEmpty()) {
					...
					queue.add(new NodeBDMSM(node.begin, mid1, node.mid, matchingList));
					queue.add(new NodeBDMSM(begin2, mid2, node.end, matchingList));
				}
			} else {
				for (IntervalBDMSM interval : node.toCompare) {
					int k = interval.begin;
	...
	public long getEuclidean(Trajectory R, Trajectory S){
		long euclidean=0;
		double[] resultT1;
		double[] resultT2;
	...
	//	System.out.println(T1.getTid());
		...
		while (!queue.isEmpty()) {
			...
			if (!node.isLeaf) {
//				System.out.println("begin: "+node.begin);
