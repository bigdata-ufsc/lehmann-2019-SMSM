...
//		ETrajectory E2 = CreateEllipseMath.createEllipticalTrajectoryFixed(S);
		...
		Deque<NodeBUMS2> queue = new ArrayDeque<>();

		Queue<IntervalBUMS2> toCompare = new ArrayDeque<>();
		toCompare.add(new IntervalBUMS2(0, (S.length() - 1)));

		NodeBUMS2 root = new NodeBUMS2(0, (E1.length() / 2), (E1.length() - 1), toCompare);

		queue.push(root);
//System.out.println("dist[]= "+Arrays.toString(dist));
		while (!queue.isEmpty()) {
			NodeBUMS2 node = queue.pop();
...
//				System.out.println("$$$ Node Start: "+dist[node.begin]+" Node End: "+dist[node.end]+" Node Mid: "+dist[node.mid]);
				...
				Queue<IntervalBUMS2> matchingList = new ArrayDeque<>();

				for (IntervalBUMS2 interval : node.toCompare) {
					...
					while (k <= interval.end) {
						if (Distance.euclidean(S.getPoint(k), E1.getEllipse(node.mid).getCenter()) <= radius) {
							...
							if (start != -1) {
								//end = k - 1;
								matchingList.add(new IntervalBUMS2(start, k-1));
							}
							start = -1;
							//end = -1;
						}
				...
				if (!matchingList.isEmpty()) {
					...
					queue.push(new NodeBUMS2(begin2, mid2, node.end, matchingList));
					queue.push(new NodeBUMS2(node.begin, mid1, mid, matchingList));
				}
			} else {
...
//		double majorAxis[] = new double[E1.length()];
		...
		toCompare.add(new IntervalBUMS2(0, (R.length() - 1)));

		root = new NodeBUMS2(0, (E2.length() / 2), (E2.length() - 1), toCompare);
...
class IntervalBUMS2 {
	...
	public IntervalBUMS2(int begin, int end) {
		this.begin = begin;
		this.end = end;
	}
}
...
class NodeBUMS2 {
	...
	int mid;
	boolean isLeaf;
	Queue<IntervalBUMS2> toCompare;

	public NodeBUMS2(int begin, int mid, int end, Queue<IntervalBUMS2> toCompare) {
		this.mid = mid;
		this.begin = begin;
		this.end = end;
